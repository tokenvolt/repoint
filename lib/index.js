...
var jsonResponseHandler = function jsonResponseHandler(response) {
...
var modifyWith = function modifyWith(methodName) {
  return _ramda2.default.curry(function (config, url, idAttributes, params) {
    ...
    return (0, _isomorphicFetch2.default)('' + config.host + buildedUrl, _extends({}, config.fetchOpts, {
      method: methodName,
      body: data,
      headers: _ramda2.default.merge(defaultHeaders(type), headers)
    })).then(config.beforeError).then(config.responseHandler).then(function (json) {
      return json;
...
var commonMethods = {
  ...
  get: _ramda2.default.curry(function (config, url, idAttributes, params) {
    ...
    return (0, _isomorphicFetch2.default)(fullUrl, _extends({}, config.fetchOpts, {
      headers: _ramda2.default.merge(defaultHeaders(type), headers)
    })).then(config.beforeError).then(config.responseHandler).then(function (json) {
      return json;
    ...
    value: function generate(name) {
      ...
      if (!singular) {
        ...
        if (nestedEndpoint !== undefined && nestedEndpoint !== null) {
          var reversedNestedNamespacedIdAttributes = nestedNamespacedIdAttributes.slice().reverse();
